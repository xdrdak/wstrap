...
function toolbox_setup() {
	...
	register_nav_menus( array(
		...
		'topnav' => __( 'Top Navigation Bar', 'toolbox' ),
		) );
...
function toolbox_posted_on() {
	printf( __( '<span class="sep">Posté le</span><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a><span class="byline"> <span class="sep"> par </span> <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s %8$s</a></span></span>', 'toolbox' ),
		esc_url( get_permalink() ),
		esc_attr( get_the_time() ),
		esc_attr( get_the_date( 'c' ) ),
		esc_html( get_the_date() ),
		esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
		esc_attr( sprintf( __( 'Voir tous les atricles de %s', 'toolbox' ), get_the_author() ) ),
...
if (!is_admin())
	add_action('wp_enqueue_scripts', 'toolbox_js');

if (!function_exists('toolbox_js')) {

	function toolbox_js() {
			// JS at the bottom for fast page loading. 
			// except for Modernizr which enables HTML5 elements & feature detects.
		
		wp_enqueue_script('modernizr', get_template_directory_uri() . '/js/vendor/modernizr.js', array('jquery'), false, false);
